...
public class SimilarityReducer extends MapReduceBase implements Reducer<Text, Text, Text, Text> {
	...
	class Predicate {
		...
		public Predicate (String input) {
			...
			this.dataSourceId = inputParams[1]; 
			this.name = inputParams[2];
			this.values = inputParams[3];
		}
			...
			for (Predicate prev : prevValues) {				
				...
				if (!prevDatasourceId.equals(currDatasourceId)) { //clean-clean ER
					if (sameBlock) { //check within the same block
						double score = linda(currPredValues, prevPredValues);						
						//System.out.println("Similarity of "+currPredicate +" and "+prevPredicate+": "+score);
						if (score > 0) {//saves great space!
							output.collect( //add the inverse in the next job's mapper
								new Text(currDatasourceId+";;;"+currPredicate), //key
								new Text(prevDatasourceId+";;;"+prevPredicate+";;;"+score)); //value	
							/*
