...
public class AttributeMapperFromEntities extends MapReduceBase implements Mapper<VIntWritable, Text, Text, Text> {
	...
	Text predToEmit = new Text();
	Text valToEmit = new Text();
...
//		}	
	...
	public void map(VIntWritable key, Text value,
		...
		if (elements.length < 2 || elements.length % 2 != 0) {
			...
			//return;
			String [] newElements = Arrays.copyOfRange(elements, 0, elements.length-1);
		...
		for (int i=0; i < elements.length; ++i) { 
			...
			predToEmit.set(pred);
			valToEmit.set(val);
			output.collect(predToEmit, valToEmit);			
		}		
	}
}
