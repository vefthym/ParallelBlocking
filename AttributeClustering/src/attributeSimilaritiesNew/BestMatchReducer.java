...
public class BestMatchReducer extends MapReduceBase implements Reducer<Text, Text, Text, VIntWritable> {
	...
	VIntWritable outputCluster = new VIntWritable();
	VIntWritable glueCluster = new VIntWritable(-1);
	...
	public void reduce(Text _key, Iterator<Text> values,
		...
		while (values.hasNext()) {
			 ...
			 reporter.progress();
			 double score = Double.parseDouble(valueParams[1]);
			 if (score > bestScore) {
				 bestScore = score;  
				 bestPredicate = new String(valueParams[0]);
			 }
		}		
		...
		if (bestScore > 0) { //FIXME: TUNE THIS THRESHOLD (INITIALLY 0)
			...
			output.collect(new Text(keyPredicateName), glueCluster); //-1 for the glue cluster
			return;
		}
		...
		if (left_cluster == null && right_cluster == null) {
			...
			outputCluster.set(clusterIndex);
			output.collect(new Text(keyPredicateName), outputCluster);
			...
			outputCluster.set(clusterIndex);
			output.collect(new Text(bestPredicateName), outputCluster);
			...
			outputCluster.set(left_cluster);
			output.collect(new Text(bestPredicateName), outputCluster);
		} else if (left_cluster == null && right_cluster != null) {
			...
			outputCluster.set(right_cluster);
			output.collect(new Text(keyPredicateName), outputCluster);
